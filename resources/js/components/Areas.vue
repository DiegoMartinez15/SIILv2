<template>
  <div class="content">
    <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
      ...
      <v-card>
        <v-card-title>
          Listado de Areas Empresariales
          <div class="flex-grow-1"></div>
          <v-text-field v-model="searchAreas" label="Buscar Areas" hide-details></v-text-field>
        </v-card-title>
        <v-data-table
          :headers="hTBAreas"
          :items="arrayAreas"
        ...
        >
          ...
          <template v-slot:top>
            <v-toolbar flat color="white">
              <div class="flex-grow-1"></div>
              <v-dialog v-model="modalArea" persistent max-width="700px">
                ...
                <v-card>
                  ...
                  <v-card-text>
                    <v-container>
                      <v-form ref="formArea" v-model="validForm" :lazy-validation="true">
                        <v-text-field
...
export default {
  ...
  computed: {
    ...
    btnTitle() {
      return this.area.id === null ? "Guardar" : "Actualizar";
    }
  },
  methods: {
    fetchAreas() {
    ...
    resetValidation() {
      let me = this;
      me.errorsNombre = [];
      me.$refs.formArea.resetValidation();
    },
    showModalEditar(area) {
      let me = this;
      me.editedArea = me.arrayAreas.indexOf(area);
      me.area = Object.assign({}, area);
      me.modalArea = true;
    },
    saveArea() {
      let me = this;
      if (me.$refs.formArea.validate()) {
        let accion = me.area.id == null ? "add" : "upd";
        me.loader = true;
        if(accion=="add"){
           axios.post('/areas/save', me.area)
            .then(function(response) {
          ...
          .catch(function(error) {
            ...
            //para actualizar
            axios.put('/areas/update',me.area)
               .then(function(response) {
    ...
    verificarAccionDato(area, statusCode, accion) {
      let me = this;
      const Toast = Swal.mixin({
        toast: true,
        position: 'top-end',
      ...
      switch (accion) {
        ...
        case "upd":
          ...
          this.fetchAreas(); 
           Toast.fire({
            icon: 'success',
            title: 'Area Actualizada con Exito'
           });
          me.loader = false;
          break;
        case "del":
      ...
      case "act":
          if (statusCode == 200) {
            try {
              me.arrayAreas[me.editedArea].estado = '1';
              //Se Lanza mensaje Final
              Toast.fire({
                icon: 'success',
                title: 'Area Activada'
